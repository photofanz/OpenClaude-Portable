...
- **The dashboard's own tools and Normal-mode approval gate don't apply on Claude Max.** The bundled proxy ignores the OpenAI-style `tools` the dashboard sends and never returns tool calls for the dashboard to execute, so its 5 tools and the per-action approval prompt are bypassed here. The proxy's own Claude Agent SDK, however, has tools enabled (`Bash`, `Read`, `Write`, `Edit`, `WebSearch`, `Grep`, `Glob`) and runs them autonomously **inside the proxy process, in the proxy's directory (`tools/claude-proxy/`, not the dashboard's working directory)**, returning only the final answer — so Claude can read/write files and run commands on Claude Max, just not through the dashboard UI and without per-action approval. The dashboard shows an info notice explaining this. For dashboard-visible, gated agent tooling, use a tool-calling provider (OpenAI Codex, OpenRouter, OpenAI, …).
