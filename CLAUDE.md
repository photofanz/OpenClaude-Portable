...
走 OpenAI-相容路徑（**不傳** `--provider`），跟 OpenRouter / DeepSeek / LM Studio / Custom 同模式 —— 原因同上一條規則。`CLAUDE_PROXY_MODE=1` 時 `start.sh` 在引擎啟動前背景起 [tools/claude-proxy/server.js](tools/claude-proxy/)（git submodule，指向 `photofanz/portable-claude-proxy`，從 `hermes-claude-proxy-v5` 衍生 — 內部用 `@anthropic-ai/claude-agent-sdk` in-process，**不 spawn CLI**），引擎結束時 kill — 跟 Ollama 同模式。Proxy log 在 `data/claude-proxy.log`，bind `127.0.0.1:3457` only。OAuth credentials 在 `data/home/.claude/.credentials.json`（HOME 已被 portable 重導涵蓋）。`claude` CLI（`@anthropic-ai/claude-code`）只用於首次 `claude login`，runtime 不需要 — 但 `start.sh` 的每日更新檢查會順手 `npm outdated @anthropic-ai/claude-code`（`--no-claude-cli-update` 可跳過）。Dashboard（[dashboard/server.mjs](dashboard/server.mjs)）的 OpenAI 路徑直接吃這個 proxy（chat 0 改動），啟動時若偵測 `CLAUDE_PROXY_MODE=1` 且 proxy 沒在跑會 self-heal（detached spawn，dashboard 結束不會 kill proxy — 會變孤兒）；**dashboard 的 tool-calling 協定在 Claude Max 下不適用** —— proxy 收到 OpenAI `tools` 直接無視（不會回 `tool_calls` 給 dashboard 執行），所以 dashboard 那 5 個工具與 Normal-mode 核可閘門都不生效；但 proxy 內部那個 `@anthropic-ai/claude-agent-sdk` 自己開了 `allowedTools: [WebSearch, WebFetch, Read, Grep, Glob, Bash(*), Write, Edit]`，會在 **proxy 行程內、proxy 的工作目錄（`tools/claude-proxy/`，不是 dashboard 選的 `WORK_DIR`）** 自主跑這些工具、只回最終文字。所以 Claude 在 Claude Max 下確實能讀寫檔/跑指令，只是不經 dashboard UI、也沒有逐動作核可。`runAgent` 偵測到 Claude Max（`isClaudeMaxProxy(cfg)`）時：`sendSSE` 一則說明這件事的提示、然後 `agentToolsEnabled=false`（不把 dashboard 工具塞進 prompt）。要 dashboard 可見、可核可的 agent 工具就用 Codex / OpenRouter / OpenAI。Dashboard 的 Setup 精靈也有 `claude-max` 卡片：按 *Save* 走 `setupOAuthProvider()` → `POST /api/setup/oauth-provider`（SSE），背後 `spawn /bin/bash tools/setup_oauth_provider.sh claude-max <model>`（= 非互動版的 `setup_claude_max`：`OPENCLAUDE_NONINTERACTIVE=1` 跳過 `read -p`、model 由參數帶入跳過選單；OAuth 'claude auth login' 仍會開瀏覽器），log 即時串到前端那個框；`exit 0` 後 server 順手 `ensureClaudeMaxProxy()` 起 proxy。
...
Dashboard（[dashboard/server.mjs](dashboard/server.mjs)）有自己的輕量 Codex 實作 `callAI_Codex`（非串流，agent mode 用）+ `streamChatResponse` 的 Codex 分支（串流，chat mode 用），都靠 `isCodexSetup(cfg)` 偵測（codex 後端 URL / `CODEX_CREDENTIAL_SOURCE=oauth` / 舊式 `AI_PROVIDER=codex`，**在 openai 分支之前判斷**）：讀 `$CODEX_HOME/auth.json`（路徑一律相對 `ROOT_DIR/data/codex/`，不信 `cfg.CODEX_HOME` 那個可能過期的絕對路徑）、token 過期就 refresh 並寫回（保留 Codex CLI 的 nested `tokens` 結構）、用 Responses API（`POST chatgpt.com/backend-api/codex/responses`，headers `Authorization Bearer` + `chatgpt-account-id` + `originator: openclaude`）。**chat 與 agent 模式都支援 Codex** —— agent mode 透過 Responses API 的 function calling：`callAI_Codex(includeTools=true)` → `buildCodexBody` 帶 `tools: toolsForCodexResponses()`（flat function 格式，不是 nested under `function`）；`messagesToCodexInput` 把 OpenAI-格式的 `assistant.tool_calls` 轉成 `function_call` input items、`role:"tool"` 轉成 `function_call_output`；回應的 `output[].type==="function_call"` 轉回 `{id:call_id, name, args}` 給 agent loop。新增 dashboard tool 時 `toolsForCodexResponses` 會自動涵蓋（同 `toolsForOpenAI` 等）。Dashboard 的 Setup 精靈也有 `codex` 卡片：跟 `claude-max` 同機制（`POST /api/setup/oauth-provider` → `spawn /bin/bash tools/setup_oauth_provider.sh codex <model>` = 非互動版 `setup_codex`），差別只在不需要 proxy。
...
- **OAuth provider setup**：`POST /api/setup/oauth-provider`（SSE）—— body `{provider:'claude-max'|'codex', model}`，`spawn /bin/bash tools/setup_oauth_provider.sh <provider> <model>`（`stdio: ['ignore','pipe','pipe']`、`OPENCLAUDE_NONINTERACTIVE=1`），把 child 的 stdout/stderr 包成 `{type:'log',line}` 串出去；child `close` code 0 → claude-max 先 `ensureClaudeMaxProxy()` → 再送 `{type:'done',success:true,config}`；`res.on('close')` 會 kill child（中斷 OAuth 只是重來）。前端對應 `setupOAuthProvider()` 讀這個 stream。Windows 直接回 error（libs 是 macOS/Linux only）。
...
- 改動 provider 設定流程時，要同步的點：[start.sh](start.sh) 的 `setup_*` 函式、[START.bat](START.bat) 對應段落、[tools/change_provider.sh](tools/change_provider.sh)（與 `.bat` peer）、[dashboard/server.mjs](dashboard/server.mjs) 的 `callAI_*`；Claude Max / Codex 兩條 OAuth 路徑的 setup 邏輯集中在 [tools/_claude_max_lib.sh](tools/_claude_max_lib.sh) / [tools/_codex_lib.sh](tools/_codex_lib.sh)（被 `start.sh`、`change_provider.sh`、`tools/setup_oauth_provider.sh` 三方 source），動它們等於同時動 CLI 與 dashboard 兩條入口 —— 但若改了 `read -p` 提示要顧到 `OPENCLAUDE_NONINTERACTIVE` 分支、改了 model 選單要顧到「caller 已帶 `CLAUDE_MAX_MODEL`/`CODEX_MODEL` 就跳過」。Dashboard 與 CLI 共用 `data/ai_settings.env`，env key 名稱不能漂移。
